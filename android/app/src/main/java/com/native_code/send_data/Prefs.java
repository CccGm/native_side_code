...
//                new UpdateNicDb(Prefs.this);
        ...
        try {
            version.setSummary(getPackageManager().getPackageInfo(PKG, 0).versionName);
    ...
    private void checkIpRange() {
        ...
        if (!matcher1.matches() || !matcher2.matches()) {
            ...
            Toast.makeText(ctxt, "error 295 Perf", Toast.LENGTH_LONG).show();
            return;
        }
        ...
        try {
            ...
            if (ipStart > ipEnd) {
                ipStartEdit.setText(before_ip_start);
                ipEndEdit.setText(before_ip_end);
                Toast.makeText(ctxt, "error 305 Perf", Toast.LENGTH_LONG).show();
            }
    ...
    private void checkPortRange() {
        ...
        try {
            ...
            if (portStart >= portEnd) {
                ...
                Toast.makeText(ctxt, "error 324 Perf", Toast.LENGTH_LONG).show();
            }
        } catch (NumberFormatException e) {
            portStartEdit.setText(before_port_start);
            portEndEdit.setText(before_port_end);
